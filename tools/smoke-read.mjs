...
try {
  ...
  // ── StatGuru answers from the read path, under a session ──
  //
  ...
  // no-model result and not a server error.
  {
    const post = async (token, body) => {
      const res = await fetch(`${BASE}/api/ai/statguru`, {
        method: "POST",
    ...
    ok("StatGuru with no session is refused", anon.status === 401, `status ${anon.status} ${JSON.stringify(anon.body)}`);
    const tok = await login("coach@example.invalid");
    const signed = await post(tok, { question: "who is the top scorer?" });
    ok("StatGuru with a session is answered by the service, not refused", signed.status === 200 && "answer" in (signed.body ?? {}), `status ${signed.status} ${JSON.stringify(signed.body)}`);
  }
} catch (e) {
