...
async function callFn(pool, authData, secret, bearer, sql, params) {
...
async function broadcastState(pool, authData, secret, bearer, hub, matchId) {
  const { rows } = await runAsPrincipal(pool, authData, secret, bearer, async client =>
...
export function sessionRoutes({ pool, authData, secret, hub }) {
  const withBroadcast = async (matchId, bearer, result) => {
    if (result.ok) await broadcastState(pool, authData, secret, bearer, hub, matchId);
    return result;
  };
  ...
  return {
    ...
    claim: async (req, res) => {
      ...
      try {
        const r = await callFn(pool, authData, secret, b, `select * from scoring_claim($1,$2)`, [id, req.body.device]);
    ...
    heartbeat: async (req, res) => {
      const { id } = req.params, b = req.headers?.authorization;
      try {
        const r = await runAsPrincipal(pool, authData, secret, b, async client => {
    ...
    armHandover: async (req, res) => {
      const { id } = req.params, b = req.headers?.authorization;
      try {
        const r = await callFn(pool, authData, secret, b,
    ...
    claimHandover: async (req, res) => {
      ...
      try {
        ...
        if (r.ok) {
          const evs = await runAsPrincipal(pool, authData, secret, b, async client =>
            client.query(`select seq, epoch, innings, payload from ball_event where match_id = $1 order by seq`, [id]));
          r.events = evs.rows;
          await broadcastState(pool, authData, secret, b, hub, id);
        }
        res.json(r);
      } catch (e) { res.status(e.status || 500).json({ error: e.code || e.message }); }
    },
    ...
    forceRelease: async (req, res) => {
      const { id } = req.params, b = req.headers?.authorization;
      try {
        const r = await callFn(pool, authData, secret, b, `select * from scoring_force_release($1)`, [id]);
        res.json(await withBroadcast(id, b, r));
      } catch (e) { res.status(e.status || 500).json({ error: e.code || e.message }); }
    },
  };
