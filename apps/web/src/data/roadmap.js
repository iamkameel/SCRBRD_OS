...
export const UPGRADES = [
  ...
  { id: "up5",  category: "AI & Analysis", priority: "high",   status: "shipped", title: "Opposition Dossier",
    desc: "The other side's squad and what the ball log says about each of them, on the fixture in Match Centre, inside the five-day window before it. Cricket columns only, figures withheld below a thirty-ball floor with the evidence named, and every read written to the other school's access log. Head-to-Head is now derived from the fixtures rather than the hand-written table it replaced, and the match-ups say how much of the log they can speak for.", effort: "Medium", walk: ["opposition", "browser-dossier"] },
  ...
  { id: "up1",  category: "AI & Analysis", priority: "medium", status: "shipped", title: "Post-Match Report",
    desc: "A printable report opened from a completed fixture in Match Centre: the result, both innings' scorecards, fall of wickets and partnerships, the match's best batting and best bowling figures, the phase breakdown the scorer's own pad already computes (packages/scoring/src/phases.mjs — it sums exactly to the innings), and a key-moments timeline of every wicket, fifty, hundred and five-wicket haul, in the order they fell. Everything on it is derived — the same fold ScorecardModal already reads (GET /matches/:id/events, deriveMatch()) and the same composed /read/phases the pad's own breakdown uses — through apps/web/src/lib/postMatchReport.js, which invents no rule of its own beside the one the package already owns. A clean @media print stylesheet (design/tokens.js) prints exactly the report, nothing else on the page.", effort: "Medium", walk: ["browser-report"] },
  { id: "up52", category: "AI & Analysis", priority: "medium", status: "shipped", title: "Season Awards / MVP",
    desc: "A season roll-up on the Leagues screen's Awards tab, scoped to a school and a team: the leading run-scorers and wicket-takers (a plain count, no floor — a season's top scorer is a fact regardless of sample size), the best batting and bowling index for whoever clears the sample floor packages/scoring/src/rating.mjs already enforces (battingIndex/bowlingIndex return null below it, and apps/web/src/lib/seasonAwards.js drops that player from the ranked list rather than showing a rating the package refused), and an MVP ranking from those same two indices — the mean of whichever exist, never a blend with zero for the discipline nobody has enough of a sample for. A season selector opens on the current school season, offers only the seasons that have figures, and keeps \"All seasons\". A season's lists come from /read/career_by_season: the same figures as /read/career, grouped by the school season each match is in (school_season_of(), the fixture list's own rule, on the Johannesburg calendar) and read through security_invoker views, so every reader sees exactly what /read/career shows them and the seasons add up to the career (db/99 §21). The sample floors apply to each season's balls alone. \"All seasons\" is /read/career and the rating package exactly as before (SCRBRD-086).", effort: "Medium", walk: ["browser-awards"] },
