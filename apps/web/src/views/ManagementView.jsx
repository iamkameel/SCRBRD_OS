...
function ManagementView({ role, users, setUsers }) {
  ...
  const TABS_MAP = {
    superadmin:    [{id:"users",icon:"users",label:"User Management",color:D.violetText},{id:"squad",icon:"bat",label:"Squad Admin",color:D.sky},{id:"fixtures",icon:"calendar",label:"Fixtures",color:D.amber},{id:"broadcast",icon:"radio-tower",label:"Broadcast",color:D.roseText},{id:"audit",icon:"search",label:"Audit Log",color:D.textMuted}],
    schooladmin:   [{id:"users",icon:"users",label:"Users",color:D.indigoText},{id:"squad",icon:"bat",label:"Squad Admin",color:D.sky},{id:"fixtures",icon:"calendar",label:"Fixtures",color:D.amber},{id:"broadcast",icon:"radio-tower",label:"Broadcast",color:D.roseText}],
    directorofsport: [{id:"squad",icon:"bat",label:"Team Management",color:D.sky},{id:"fixtures",icon:"calendar",label:"Fixture Admin",color:D.amber},{id:"broadcast",icon:"radio-tower",label:"Announcements",color:D.roseText}],
    coach:         [{id:"squad",icon:"bat",label:"Squad Tools",color:D.emerald}],
    facilities:    [{id:"grounds",icon:"ground",label:"Ground Tasks",color:D.teal}],
  };
      ...
      <div style={{position:"fixed",inset:0,zIndex:600,background:T.glass.scrim,backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
        <div style={{width:"100%",maxWidth:"480px",borderRadius:D.xl,border:`1px solid ${D.borderMed}`,background:D.surf1,overflow:"hidden",boxShadow:T.elevation.xl}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 20px",borderBottom:`1px solid ${D.border}`,background:`${D.violet}08`}}>
            <div style={{fontFamily:D.head,fontSize:"15px",fontWeight:800,color:D.textPrimary}}>{editUser?<><Icon name="pencil"/> Edit User</>:<><Icon name="plus"/> Add New User</>}</div>
          ...
          <div style={{padding:"20px",display:"flex",flexDirection:"column",gap:"14px"}}>
            ...
            {u.role&&<div style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 14px",borderRadius:D.md,background:`${ROLES[u.role]?.color||D.indigo}12`,border:`1px solid ${ROLES[u.role]?.color||D.indigo}33`}}>
              <span style={{fontSize:"16px",color:ROLES[u.role]?.color||D.indigo}}><Icon name={ROLES[u.role]?.icon}/></span>
              ...
              {mayGrantRole(u.role,"superadmin")&&<span style={{marginLeft:"auto",fontFamily:D.head,fontSize:"8px",color:D.roseText}}><Icon name="triangle-alert"/> Highest privilege</span>}
            </div>}
              ...
              <div style={{display:"grid",gridTemplateColumns:"var(--g-2,1fr 1fr)",gap:"6px",maxHeight:"200px",overflowY:"auto",paddingRight:"2px"}}>
                {availableRoles.map(([r,rc])=>(
                  <button key={r} onClick={()=>setU(prev=>({...prev,role:r}))} className="pressBtn" style={{
                    ...
                    <span style={{fontSize:"14px",color:u.role===r?rc.color:D.textMuted}}><Icon name={rc.icon}/></span>
    ...
    return (
      <div style={{position:"fixed",inset:0,zIndex:700,background:T.glass.scrim,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
        <div style={{width:"100%",maxWidth:"360px",borderRadius:D.xl,border:`1px solid ${D.rose}44`,background:D.surf1,padding:"24px",textAlign:"center"}}>
          <div style={{fontSize:"36px",marginBottom:"12px",color:D.amber}}><Icon name="triangle-alert"/></div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",alignItems:"center"}}>
            ...
            <div style={{position:"relative",flex:1,minWidth:"180px"}}>
              <span style={{position:"absolute",left:"10px",top:"50%",transform:"translateY(-50%)",fontSize:"12px",color:D.textMuted}}><Icon name="search"/></span>
            ...
            <select value={filterRole} onChange={e=>setFilterRole(e.target.value)}
              ...
              {Object.entries(ROLES).map(([r,rc])=><option key={r} value={r}>{rc.label}</option>)}
            </select>
            {/* Status filter */}
                      ...
                      <tr key={u.id} style={{borderTop:`1px solid ${D.border}`,background:i%2===0?"transparent":D.surf2+"22",opacity:u.status==="suspended"?0.55:1,transition:"opacity .2s"}}>
                        ...
                        <td style={{padding:"10px 14px"}}>
                          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                            <div style={{width:"30px",height:"30px",borderRadius:"50%",background:`${rc2.color||D.indigo}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",flexShrink:0,color:rc2.color||D.indigo}}><Icon name={rc2.icon||"user"}/></div>
                            <div>
                        ...
                        <td style={{padding:"10px 14px"}}>
                          {canManageUsers ? (
                            ...
                            <span style={{padding:"3px 9px",borderRadius:D.pill,background:`${rc2.color||D.indigo}18`,border:`1px solid ${rc2.color||D.indigo}33`,fontFamily:D.head,fontSize:"9px",fontWeight:700,color:rc2.color||D.indigo}}>
                              <Icon name={rc2.icon||"user"}/> {rc2.label}
                            </span>
                          )}
                        </td>
                        {/* Email */}
                        ...
                        <td style={{padding:"10px 14px"}}>
                          <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                            {canManageUsers&&(
                              <>
                                <button onClick={()=>{setEditUser({...u});setAddOpen(false);}} className="pressBtn" style={{padding:"4px 10px",borderRadius:D.md,cursor:"pointer",background:D.surf3,border:`1px solid ${D.border}`,fontFamily:D.head,fontSize:"9px",fontWeight:700,color:D.textSecondary}}><Icon name="pencil"/> Edit</button>
                                <button onClick={()=>toggleStatus(u.id)} className="pressBtn" style={{padding:"4px 10px",borderRadius:D.md,cursor:"pointer",background:"transparent",border:`1px solid ${D.border}`,fontFamily:D.head,fontSize:"9px",fontWeight:700,color:u.status==="active"?D.amber:D.emerald}}>
                                  {u.status==="active"?<><Icon name="pause"/> Suspend</>:<><Icon name="play"/> Restore</>}
                                </button>
                                {u.role!=="superadmin"&&<button onClick={()=>setDelConf(u.id)} className="pressBtn" aria-label={`Delete ${u.name}`} title="Delete" style={{padding:"4px 10px",borderRadius:D.md,cursor:"pointer",background:"transparent",border:`1px solid ${D.rose}44`,fontFamily:D.head,fontSize:"9px",fontWeight:700,color:D.roseText}}><Icon name="trash-2"/></button>}
                              </>
                            )}
                          </div>
                        </td>
          ...
          {!canManageUsers&&<div style={{padding:"12px 16px",borderRadius:D.md,background:`${D.amber}0a`,border:`1px solid ${D.amber}22`,fontFamily:D.body,fontSize:"12px",color:D.amber}}><Icon name="triangle-alert"/> Only Super Admin can create, edit or delete users. Role changes require Super Admin privileges.</div>}
        </div>
      )}
          ...
          {MATCHES.filter(m=>m.status==="upcoming").map(m=>(
            <div key={m.id} style={{borderRadius:D.lg,border:`1px solid ${D.border}`,background:D.surf1,padding:"14px 18px",display:"flex",alignItems:"center",gap:"14px",flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                ...
                {isAdmin&&<button className="pressBtn" style={{padding:"3px 10px",borderRadius:D.pill,cursor:"pointer",background:"transparent",border:`1px solid ${D.border}`,fontFamily:D.head,fontSize:"9px",color:D.textMuted}}><Icon name="pencil"/> Edit</button>}
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab==="audit"&&holdsCapability(role,"audit.read")&&(
        <div style={{borderRadius:D.lg,border:`1px solid ${D.border}`,background:D.surf1,overflow:"hidden"}}>
          <div style={{padding:"12px 18px",borderBottom:`1px solid ${D.border}`}}><div style={{fontFamily:D.head,fontSize:"12px",fontWeight:700,color:D.textPrimary}}><Icon name="search"/> Audit Log</div></div>
          {AUDIT_LOG.map((a,i)=>{
      ...
      {activeTab==="grounds"&&(
        <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{fontFamily:D.head,fontSize:"12px",fontWeight:700,color:D.textPrimary}}><Icon name="ground"/> Ground Tasks</div>
